...
```bash
# dune exec -- my_lab3 --{interpolation_name} --step {step_number} [-n {number point}] 
...
Проверяется корректность вычисления значения на отрезке между двумя точками:

* Работа на простом случае y = x (step = 0.5)
...
* Работа на общем сдвинутом случае y = 2x + 1 (step = 1)
...
Проверяется корректность вычисления на параболе y = x² (step = 0.5; n = 4):
...
Проверяется корректная логика скользящего окна в модели y = x (step = 0.5): 
